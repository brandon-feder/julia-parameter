...
C = complex(0.1, -0.1) # The chosen number
...
VIEW_CONSTANT = 1.5 # A constant factor by which the viewport is scaled

# Calculate the number of iterations before z0 escapes
@jit(int32(complex64, complex64))
def iteratePoint(z0, C):
    z = z0
...
@guvectorize([(complex64[:], complex64[:], int32[:])], '(n),()->(n)', target='parallel')
def calcPoints(z0, C, it):
    for k in range(z0.shape[0]):
        it[k] = iteratePoint(z0[k], C[0])
...
def calcJuliaSet(C):
    ...
    ITERATIONS = calcPoints(Z0, np.array(C))
...
def setupViewportAndSamples():
    # Setup viewport
    global VIEW_RATIO, CENTER, VIEW_WIDTH, VIEW_HEIGHT, WIDTH, HEIGHT, MAIN_SURFACE, Z0
...
def run(width, height, pipe):
    # Initial setup
    global VIEW_RATIO, WIDTH, HEIGHT, WINDOW, MAIN_SURFACE, FONT, CLOCK, PIXELS, RUNNING, C

    WIDTH = width
    HEIGHT = height

    pygame.display.init()
    pygame.display.set_caption('The Corresponding Julia Set')
    pygame.font.init()
    ...
    while RUNNING:
        ...
        if RUNNING == False:
            print("Info: Julia process quiting after next frame")
